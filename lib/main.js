...
var main = function main(command, file, prompt) {
  ...
  if (command) {
    return interpreter.loadStandardLib().then(function () {
      return console.log(getResponse(interpreter, command));
    });
  }
  ...
  var read = function read() {
    return rl.ask(interpreter.prompt).then(function (input) {
      ...
      input.split('\n').forEach(function (line) {
        return console.log(getResponse(interpreter, line).toString());
      });

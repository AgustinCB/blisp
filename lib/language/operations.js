...
var processList = function processList(list) {
  ...
  if (first_list && list.length === 1 && (list[0].length !== undefined && typeof list[0] !== 'string' || list[0] instanceof _symbol2.default)) {
    list = list[0] instanceof _symbol2.default ? list[0].value : list[0];
  }
  if (first_list && list instanceof _expressions.QExpression) {
    return list.list;
  }
  if (first_list && list instanceof _expressions.SExpression) {
    list = list.run();
  }
  if (list.length === undefined) return [list];
  return list.map(function (item) {
    if (item instanceof _expressions.QExpression) return item.list;
...
var list = exports.list = {
  ...
  eval: function _eval() {
    ...
    if (args[0] instanceof _expressions.QExpression) {
      args[0] = args[0].list;
    }

    if (args[0].length !== undefined && typeof args[0] !== 'string' && typeof args[0] !== 'function') {
      return new _expressions.SExpression(args[0]).run();
    }

    if (args.length > 1) {
      return new _expressions.SExpression(args).run();
    }
...
var def = exports.def = function def() {
  ...
  var symbols = function () {
    ...
    if (args[0] instanceof _expressions.SExpression && !(args[0] instanceof _expressions.QExpression)) return args[0].run();
    return args[0];
  }(),
      values = args.slice(1);
...
var func = exports.func = function func() {
  ...
  if (!(arg_list instanceof _expressions.QExpression) || !(body instanceof _expressions.QExpression)) {

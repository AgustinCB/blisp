...
var processElement = function processElement(execution, item) {
...
var conditionals = exports.conditionals = {
  'if': function _if() {
    ...
    if (processElement(true, args[0])) return args[1].run();else if (args[2]) return args[2].run();
  },
  unless: function unless() {
    var args = [].concat(Array.prototype.slice.call(arguments));
    return conditionals.if.apply(conditionals, [!args[0]].concat(_toConsumableArray(args.slice(1))));
  }
};
...
var def = exports.def = function def() {
  ...
  var symbols = function () {
    if (args[0] instanceof _symbol2.default) return args[0].value;
  ...
  symbols.list.forEach(function (symbol, index) {
    var val = values[index].constructor === _expressions.QExpression ? values[index] : processElement(true, values[index]);
    // console.log('def', symbol.name, val)
    _environment2.default.set(symbol.name, val, env);
  });
};
...
var func = exports.func = function func() {
  ...
  console.log('prev', args, _environment2.default.namespaces.get('global'));
  ...
  if (!argList.list.reduce(function (acc, next) {
  ...
  var dynArgs = argList.list.filter(function (symbol) {
  ...
  console.log('prev', 'body', body, 'argList', argList, _environment2.default.namespaces.get('global'));
  var result = function result() {
    console.log('next');
    var extraArgs = dynArgs[0];
    ...
    argList.list.forEach(function (symbol, index) {

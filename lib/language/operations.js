...
var processElement = function processElement(item) {
...
var conditionals = exports.conditionals = {
  'if': function _if() {
    ...
    if (processElement(args[0])) return args[1].run();else if (args[2]) return args[2].run();
  },
...
var def = exports.def = function def() {
  ...
  symbols.list.forEach(function (symbol, index) {
    var val = values[index].constructor === _expressions.QExpression ? values[index] : processElement(values[index]);
...
var func = exports.func = function func() {
  ...
  if (!(arg_list instanceof _expressions.QExpression) || !(body instanceof _expressions.SExpression)) {
    console.log('arg_list', arg_list, 'body', body, args, _environment2.default.namespaces.get('global').get('curry'));

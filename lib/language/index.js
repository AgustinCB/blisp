...
var list = parsec.lazy(function () {
  ...
  return (_builtins$or$trim$man = builtins.or(grammar.types.boolean, grammar.types.symbol, grammar.types.string, factor).trim().many(Array)).between.apply(_builtins$or$trim$man, _toConsumableArray(grammar.chars.parenthesis)).then(function (elements) {

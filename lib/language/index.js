...
var list = parsec.lazy(function () {
  ...
  return (_builtins$or$trim$man = builtins.or(grammar.types.boolean, grammar.types.symbol, grammar.types.string, grammar.types.float, parsec.int, unevaluatedStatment, evaluatedStatment).trim().many(Array)).between.apply(_builtins$or$trim$man, _toConsumableArray(grammar.chars.parenthesis));
});

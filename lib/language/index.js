...
var factor = parsec.lazy(function () {
  return list.or(unevaluatedStatment);
});

var list = parsec.lazy(function () {
  var _builtins$or$trim$man;

  return (_builtins$or$trim$man = builtins.or(grammar.types.boolean, grammar.types.symbol, grammar.types.string, parsec.int, factor).trim().many(Array)).between.apply(_builtins$or$trim$man, _toConsumableArray(grammar.chars.parenthesis)).then(function (elements) {

...
var statment = exports.statment = parsec.lazy(function () {
  return evaluatedStatment.or(unevaluatedStatment, grammar.comment);
...
var list = parsec.lazy(function () {
  ...
  return (_builtins$or$trim$man = builtins.or(grammar.types.boolean, grammar.keywords.nil, grammar.types.symbol, grammar.types.string, grammar.types.float, parsec.int, unevaluatedStatment, evaluatedStatment).trim().many(Array)).between.apply(_builtins$or$trim$man, _toConsumableArray(grammar.chars.parenthesis));
});

var unevaluatedStatment = grammar.chars.singleQuote.then(list).then(function (elements) {
  return new _expressions.QExpression(elements);
});
var evaluatedStatment = list.then(function (elements) {

...
var statment = exports.statment = parsec.lazy(function () {
  return evaluatedStatment.or(grammar.comment);
});
...
var factors = parsec.lazy(function () {
  return builtins.or(grammar.types.boolean, grammar.types.symbol, grammar.types.string, parsec.int, list, unevaluatedStatment).trim().many(Array);
});

var list = factors.between.apply(factors, _toConsumableArray(grammar.chars.parenthesis)).then(function (elements) {
  return new _expressions.SExpression(elements);
}).trim();

var unevaluatedStatment = grammar.chars.singleQuote.then(list).then(function (sexp) {
